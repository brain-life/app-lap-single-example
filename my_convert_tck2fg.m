...
function fg = dtiImportFibersMrtrix_local(filename, step, fiberPointStride)
%
...
try
    ...
    if (~isempty(strmatch('step_size:',header)))
       ...
       stepSize = step;
       warning([ 'Could not read stepSize from .tck file header, assuming stepSize=' num2str(step) ]);
    end
